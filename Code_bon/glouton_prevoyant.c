...
char glouton_prevoyant(char perso, char *tableau, int taille){ //va choisir la couleur qui est en majorité à sa disposition
  ...
  for(k=0;k<compte;k++){
	...
	for (m=0;m<7;m++){
		if (couleur_compte[m]>nb_max){
			nb_max=couleur_compte[m];
		}
	}
